...
% Finding the dot product through filter

for n = 1:batch_size
    input_n.data = input.data(:, n);
    col = im2col_conv(input_n, layer, h_out, w_out);
    col = reshape(col, k*k*c, h_out*w_out);
%         disp(param.w(:,n)'*col(:,n))
    data(:,1:num) = transpose(col)*param.w+param.b;
    output.data(:,n) = data(:);
end
    
%                  output.data(h,w,ch,n) = transpose(param.w(:,w,ch))*col(:,w,ch) + transpose(param.b);
%                  output.data
